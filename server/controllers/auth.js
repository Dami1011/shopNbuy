...
export const registerUser = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ access_token, user, msg: "User registration sucessful" });
  } catch (error) {
...
export const loginUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!isPasswordMatch) return next(customError(400, "Invalid Passwrod"));
...
export const getSingleUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) return next(customError(400, "cant't find user"));
...
export const updateUser = async (req, res) => {
  ...
  try {
    if (userId) {
      ...
      res.status(404);
      throw new Error("User profile not updataed");
    }
  } catch (error) {
    res.status(500).json(error);
  }
