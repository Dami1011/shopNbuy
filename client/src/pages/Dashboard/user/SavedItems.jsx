...
export default function SavedItems() {
  ...
  const addToBag = (item) => {
    ...
    toast.success(`${item.title} add to bag`);
    navigate("/bag");
  };
  ...
  const deleteSavedProduct = async (item) => {
    try {
      ...
      toast.success(`Product successfully removed from saved items`);
      navigate(0);
    } catch (error) {
      console.log(error);
      ...
      {loading ? (
        ...
        <>
          {data?.length > 0 ? (
                ...
                >
                  <Col md={8} className="mb-4">
                    <div className="d-flex align-items-center gap-3">
                      ...
                      <div>
                        <Link to={`/collections/${item.category}/${item.slug}`}>
                          <span className="fs-5 text-black">{item.title}</span>
                        </Link>
                        <div>
            ...
            <h1 className="fs-5">Sorry, you have not liked a product yet</h1>
          )}
        </>
      )}
    </>
